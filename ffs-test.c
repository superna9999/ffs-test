 ...
 *
 * Modified by Alan Ott to support configurable endpoint lengths for ELC
 * 2014.
 *
...
static const struct {
	...
	struct {
		...
		struct usb_endpoint_descriptor_no_audio sink_iso_alt;
		struct usb_endpoint_descriptor_no_audio source_iso_alt;
#endif
...
static struct thread {
	...
	{
		"ep1", 8 * 1024, /* overwritten in main() */
		fill_in_buf, "<in>",
		write_wrap, NULL,
		0, 0, NULL, 0
	},
...
int main(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (buf_size <= 0) {
			fprintf(stderr, "transfer_size invalid. Must be > 0.\n");
			return 1;
		}
